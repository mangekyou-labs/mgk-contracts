...
const providers = {
  hederaTestnet: new ethers.providers.JsonRpcProvider(HEDERA_TESTNET_URL)
}

const signers = {
  hederaTestnet: new ethers.Wallet(HEDERA_TESTNET_PRIVATE_KEY).connect(providers.hederaTestnet)
}
...
async function getFrameSigner(options) {
  try {
    const frame = new ethers.providers.JsonRpcProvider(HEDERA_TESTNET_URL)
